...
class _GameQuestionFormPageState extends State<_GameQuestionFormPage> {
  ...
  late final TextEditingController _finalConsonantCtrl;
  ...
  void initState() {
    ...
    _finalConsonantCtrl = TextEditingController(text: ad['finalConsonant']?.toString() ?? '');
    _imageUrlCtrl = TextEditingController(text: ad['imageUrl']?.toString() ?? ad['image']?.toString() ?? '');
    _audioUrlCtrl = TextEditingController(text: ad['audioUrl']?.toString() ?? ad['audio']?.toString() ?? '');

    _romanizedCtrl = TextEditingController(text: ad['romanized']?.toString() ?? '');
    _emojiCtrl = TextEditingController(text: ad['emoji']?.toString() ?? ad['emoji']?.toString() ?? '');
    ...
    _finalConsonantCtrl.addListener(() => setState(() {}));
      ...
      children: [
        ...
        Row(
          ...
          children: [
            GestureDetector(
              ...
              child: Container(
                ...
                child: _uploadingImg
                    ...
                    : _imageUrlCtrl.text.isNotEmpty && _imageUrlCtrl.text.startsWith('http')
                        ? ClipRRect(
                            borderRadius: BorderRadius.circular(14.r),
                            child: Image.network(
                              AuthService.getOptimizedImageUrl(_imageUrlCtrl.text, width: 200),
                              fit: BoxFit.cover,
                        ...
                        : Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              Icon(Icons.add_photo_alternate_rounded, color: color, size: 24.sp),
                              SizedBox(height: 2.h),
                              Text(
                                'Chọn ảnh',
                                style: GoogleFonts.plusJakartaSans(
            ...
            Expanded(
              child: _prettyField(
                label: 'Đường dẫn ảnh minh họa (imageUrl)',
                ctrl: _imageUrlCtrl,
        ...
        Row(
          ...
          children: [
              ...
              onTap: _uploadingAudio ? null : () async {
                ...
                try {
                  ...
                  if (url != null) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Tải âm thanh phát âm thành công!')),
                    );
                  } else {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Tải âm thanh thất bại')),
                    );
                  }
                } catch (e) {
                  ScaffoldMessenger.of(context).showSnackBar(
                ...
                child: _uploadingAudio
                    ...
                    : _audioUrlCtrl.text.isNotEmpty
                        ...
                        : Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              Icon(Icons.audiotrack_rounded, color: color, size: 24.sp),
                              SizedBox(height: 2.h),
                              Text(
                                'Chọn âm',
                                style: GoogleFonts.plusJakartaSans(
            ...
            Expanded(
              child: _prettyField(
                label: 'Đường dẫn âm thanh phát âm (audioUrl)',
                ctrl: _audioUrlCtrl,
                hint: 'Dán link hoặc chọn file audio để upload',
  ...
  Future<void> _onSave() async {
    ...
    if (widget.gameKey == 'letter_catch') {
      ...
      ad['finalConsonant'] = _finalConsonantCtrl.text.trim();
      ad['imageUrl'] = _imageUrlCtrl.text.trim();
      ad['audioUrl'] = _audioUrlCtrl.text.trim();
